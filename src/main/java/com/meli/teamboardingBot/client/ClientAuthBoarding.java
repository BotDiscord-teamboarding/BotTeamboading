...
public class ClientAuthBoarding {
    ...
    private final String googleAuthUrl = "/auth/google_login";
    ...
    public AuthTokenResponseDTO getAccessTokenWithGoogle(String authorizationCode) {
        String url = apiUrl + googleAuthUrl + "?code=" + authorizationCode;
        ...
        logger.info("Autenticando com Google via: {}", url);
        ...
        try {
            ...
            long duration = System.currentTimeMillis() - startTime;
            logger.info("Autenticação Google realizada com sucesso em {}ms", duration);
            ...
            long duration = System.currentTimeMillis() - startTime;
            logger.error("Timeout na autenticação Google após {}ms: {}", duration, e.getMessage());
            throw new RuntimeException("Timeout na conexão com a API (" + duration + "ms): " + e.getMessage(), e);
        } catch (Exception e) {
            long duration = System.currentTimeMillis() - startTime;
            logger.error("Erro na autenticação Google após {}ms: {}", duration, e.getMessage());
